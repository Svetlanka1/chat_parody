...
class ChatApp:
    def __init__(self, root):
        ...
        self.window.title("Message Parody")
        ...
        self.captcha_frame = tk.Frame(self.window)
        self.captcha_frame.config(bg="#B0C4DE")
    ...
    def start_chat(self):
        ...
        # Заголовок чата
        title_label = tk.Label(self.chat_frame, text=friend_title, font=("Arial", 16), bg="#B0C4DE")
        ...
        self.reply_button = tk.Button(
            self.chat_frame,
            text="Ответить",
            command=lambda: self.send_standard_reply(self.messages_area, self.reply_button)
        )
        self.reply_button.pack(pady=10)

        self.show_frame(self.chat_frame)

    def send_standard_reply(self, messages_area, reply_button):
        ...
        friend_response = "Супер! Давай я и тебе настроение подниму) Чем хочешь заняться?"

        # Задержка перед появлением ответа друга
        self.window.after(1000, lambda: self.display_friend_response(friend_response, messages_area, reply_button))

    def display_friend_response(self, response, messages_area, reply_button):
        friend_title = "Лучший друг" if self.selected_gender == "female" else "Лучшая подруга"
        messages_area.config(state='normal')
        ...
        laugh_button = tk.Button(self.chat_frame, text="Похихикать",
                                 command=lambda: self.choose_to_laugh(messages_area))
        play_button = tk.Button(self.chat_frame, text="Поиграть", command=lambda: self.choose_to_play(messages_area))

        # Располагаем кнопки
        laugh_button.place(x=300, y=500)
        play_button.place(x=400, y=500)

    def choose_to_laugh(self, messages_area):
        friend_title = "Лучший друг" if self.selected_gender == "female" else "Лучшая подруга"
        messages_area.config(state='normal')
        ...
        general_button = tk.Button(self.chat_frame, text="Общие", command=lambda: self.tell_joke(1, messages_area))
        knock_knock_button = tk.Button(self.chat_frame, text="Тук-тук",
                                       command=lambda: self.tell_joke(2, messages_area))
        programming_button = tk.Button(self.chat_frame, text="Программистские",
                                       command=lambda: self.tell_joke(3, messages_area))

        # Устанавливаем точные координаты для кнопок
        general_button.place(x=250, y=500)
        knock_knock_button.place(x=350, y=500)
        programming_button.place(x=450, y=500)

    def choose_to_play(self, messages_area):
        friend_title = "Лучший друг" if self.selected_gender == "female" else "Лучшая подруга"
        messages_area.config(state='normal')
        ...
        # Создаем кнопки для выбора действия
        rock_button = tk.Button(self.chat_frame, text="Камень",
                                command=lambda: self.play_rock_paper_scissors("Камень", messages_area))
        paper_button = tk.Button(self.chat_frame, text="Бумага",
                                 command=lambda: self.play_rock_paper_scissors("Бумага", messages_area))
        scissors_button = tk.Button(self.chat_frame, text="Ножницы",
                                    command=lambda: self.play_rock_paper_scissors("Ножницы", messages_area))

        # Располагаем кнопки
        rock_button.place(x=280, y=500)
        paper_button.place(x=360, y=500)
        scissors_button.place(x=440, y=500)

    def play_rock_paper_scissors(self, user_choice, messages_area):
        ...
        messages_area.config(state='normal')
        messages_area.insert(tk.END, f"\nТы выбрал(а) {user_choice}, а я выбрал(а) {computer_choice}. {result}\n\n")
        messages_area.see(tk.END)
        messages_area.config(state='disabled')
    ...
    def tell_joke(self, joke_type, messages_area):
        try:
            setup, punchline = self.fetch_and_translate_joke(joke_type)
            friend_title = "Лучший друг" if self.selected_gender == "female" else "Лучшая подруга"
            messages_area.config(state='normal')
            messages_area.insert(tk.END, f"\n{friend_title}: {setup}\n{punchline}\n\n")
            messages_area.see(tk.END)
    ...
    def ask_what_next(self, messages_area):
        ...
        laugh_button = tk.Button(self.chat_frame, text="Похихикать", command=lambda: self.choose_to_laugh(messages_area))
        play_button = tk.Button(self.chat_frame, text="Поиграть", command=lambda: self.choose_to_play(messages_area))
    ...
    def fetch_and_translate_joke(self, joke_type):
        """Получает случайную шутку по типу и переводит ее на русский язык"""
        if joke_type == 1:

...
class ChatApp:
    def __init__(self, root):
        ...
        self.label_primer = tk.Label(self.math_frame, text="", font=("Arial", 24))
        self.label_primer.pack(pady=100)
    ...
    def math_primer(self):
        # Создаем пример, чтобы проверить интеллект пользователя
        num1 = random.randint(1, 9) # генерируем числа из указанного промежутка,
        num2 = random.randint(1, 9) # чтобы пример был поистине сложным
        question_text = f"Проверим ваши базовые знания математики.\n Введите сумму {num1} и {num2}."
        self.label_primer.config(text=question_text, bg="#B0C4DE")
    ...
    def ask_gender(self):
        ...
        male = tk.Button(self.gender_frame, text="Мужчина", command=lambda: select_gender("male"),
                                font=("Arial", 18), width=15)
        female = tk.Button(self.gender_frame, text="Женщина", command=lambda: select_gender("female"),
                                  font=("Arial", 18), width=15)
    ...
    def send_reply(self, messages_area, reply_button):
        ...
        user_answ = "Привет, ничего, а ты?"

        # Добавляем сообщение пользователя
        messages_area.config(state='normal')
        messages_area.insert(tk.END, f"Вы: {user_answ}\n\n")
    ...
    def what_next(self, messages_area):
        friend_title = "Лучший друг" if self.selected_gender == "female" else "Лучшая подруга"
        messages_area.config(state='normal')
